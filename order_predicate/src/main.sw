...
fn handle_redeem_plus_cancel_case() -> bool {
    ...
    // USDT Coint (Output 7) = COMMISSION (checks in the redemption predicate)
...
fn handle_redeem_case() -> bool {
    ...
    // 2. 2nd output is of type Change - (USDT)
    // 3. 3rd output is of type Coin - (YES / NO) Winning Asset
    // 4. 4th output is of type Coin - (USDT)
    // 5. 5th output is of type Coin - (USDT) - to ORACLE as Commission
    // 6. 6th output is of type Change - (ETH)

    // The checks on output_to are : 
    // USDT Coin (Output 4) == YES/NO Coin (Output 3)
    // USDT Coin (Output 4) must go to the USER
    ...
    && validate_output(2, USDT, 2) && (validate_output(3, YES, 0) || validate_output(3, NO, 0))
    && validate_output(4, USDT, 0) && validate_output(5, USDT, 0) 
...
fn handle_cancellation_case() -> bool {
    ...
    // USDT Coin (Output 4) must go to USER
    ...
    && validate_output(4, USDT, 0) && validate_output(5, YES, 0)
